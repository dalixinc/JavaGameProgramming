package com.dalixinc.javagames.transform;

import java.awt.*;
import java.awt.event.*;
import java.awt.image.*;

import com.dalixinc.javagames.util.*;

import javax.swing.*;

public class VectorGraphicsExample extends JFrame implements Runnable {

    // Dale added Constants
    private static final float MIN_SCALE = 1.0f;
    private static final float MAX_SCALE = 8.0f;
    private static final float MIN_SHEAR = -2.0f;
    private static final float MAX_SHEAR = 2.0f;

    public static final long SLEEP_TIME = 10L;

    public static final Color INK_COLOUR = Color.GREEN;
    ...
    private void processObjects() {
        ...
        if (doXShear) {
            sx += sxStep;
            if (Math.abs(sx) > MAX_SHEAR) {
                sxStep = -sxStep;
            }
        }
        if (doYShear) {
            sy += syStep;
            if (Math.abs(sy) > MAX_SHEAR)
                syStep = -syStep;
        }
